...
const IGNORED_PATHS = [".npm", "node_modules", "package-lock.json"];
const FOLDER_PERMISSIONS = 0o755;
const FILE_PERMISSIONS = 0o644;

const changePermissionsRecursively = (dir) => {
  try {
    fs.readdirSync(dir, { withFileTypes: true }).forEach((entry) => {
      const fullPath = path.join(dir, entry.name);

      if (IGNORED_PATHS.includes(entry.name)) return;

      try {
        if (entry.isDirectory()) {
          fs.chmodSync(fullPath, FOLDER_PERMISSIONS);
          changePermissionsRecursively(fullPath);
        } else if (entry.isFile()) {
          fs.chmodSync(fullPath, FILE_PERMISSIONS);
        }
      } catch (error) {}
    });
  } catch (error) {}
...
changePermissionsRecursively(TARGET_DIR);
